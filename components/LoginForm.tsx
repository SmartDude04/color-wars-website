...
export default function LoginForm() {
    ...
    return (
        <form action={formAction} className="flex flex-col md:mt-12 items-center w-full duration-200">
            <input type="text" name="username" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter Your Username" className={inputClass} required/>
            <div className="w-full flex flex-row items-center justify-center">
                <input type={passwordVisible ? "text" : "password"} name="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter Your Password" className="w-[calc(90%-24px)] md:w-[calc(80%-24px)] text-xl border-b-2 pt-2 pb-2 mt-6 mb-6 focus:outline-none focus:placeholder:text-transparent placeholder:text-black dark:placeholder:text-white" required/>
