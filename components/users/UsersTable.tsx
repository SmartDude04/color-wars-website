...
export default function UsersTable({ users, changeUserRole, deleteUser, curUserId }: Props) {
    return (
        <table className="w-[95%] md:w-[70%] border-separate border-spacing-0 mt-6 border-1 border-gray-300 dark:border-[#333333] bg-white rounded-xl duration-200 text-lg mb-8">
            ...
            <tr>
                <th className={`text-left p-4 rounded-tl-xl ${users.length === 0 ? "rounded-bl-xl" : ""}`}>Username</th>
                <th className="text-left p-4">Role</th>
                <th className={`text-left p-4 rounded-tr-xl ${users.length === 0 ? "rounded-br-xl" : ""}`}>Delete</th>
            </tr>
            </thead>
