...
export default function VerifyUsersTable({ unverifiedUsers, verifyUser, deleteUser, loading }: Props) {
    return (
        <table className="w-[95%] md:w-[70%] border-separate border-spacing-0 mt-6 border-1 border-gray-300 dark:border-[#333333] bg-white rounded-xl duration-200 text-lg">
            <thead className="bg-gray-200 dark:bg-[#202020] border-0">
                <tr>
                    <th className={`text-left p-4 rounded-tl-xl ${unverifiedUsers.length === 0 && !loading ? "rounded-bl-xl" : ""}`}>Username</th>
                    <th className={`text-left p-4 rounded-tr-xl ${unverifiedUsers.length === 0 && !loading ? "rounded-br-xl" : ""}`}>Action</th>
                </tr>
            </thead>
